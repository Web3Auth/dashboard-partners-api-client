...
import { Response, GetOrganizationsResponse, ErrorResponse, CreateProjectRequest, CreateProjectResponse, GetProjectResponse, GetProjectsResponse, UpdateProjectRequest, GetChainConfigsResponse, GetWhitelistsResponse, CreateWhitelistRequest, DeleteWhitelistRequest, GeneralResponse } from "./types";
...
const sign = (method: string, url: string, data: string) => {
  const b = Buffer.concat([Buffer.from(method, "utf8"), Buffer.from(url, "utf8"), Buffer.from(data, "utf8")]);
...
async function getOrganizations() {
  ...
  try {
      const res: Response<GetOrganizationsResponse> = await client.request({ method, url, headers: { "X-Auth-Signature": signature } });
      const { data } = res;
...
async function createProject() {
  ...
  try {
      const res: Response<CreateProjectResponse> = await client.request({ method, url, data, headers: { "X-Auth-Signature": signature } });
...
async function getProject() {
  ...
  try {
      const res: Response<GetProjectResponse> = await client.request({ method, url, headers: { "X-Auth-Signature": signature } });
      const { data } = res;
...
async function getProjects() {
  ...
  const url = `${urlPath}?team_id=${organizationId}`;
  try {
      const res: Response<GetProjectsResponse> = await client.request({ method, url, headers: { "X-Auth-Signature": signature } });
      const { data } = res;
...
async function updateProject() {
  ...
  const signature = sign(method, url, JSON.stringify(data));
  try {
      const res: Response<GeneralResponse> = await client.request({ method, url, data, headers: { "X-Auth-Signature": signature } });
...
async function getChainConfigs() {
  ...
  const url = `${urlPath}?team_id=${organizationId}`;
  try {
      const res: Response<GetChainConfigsResponse> = await client.request({
          method,
          url,
          headers: { "X-Auth-Signature": signature }
      });
      console.log("getChainConfigs - response:", res.status, res.data.chain_namespaces);
  } catch (error) {
    const errorData = (error as AxiosError).response?.data as ErrorResponse;
    console.error("getChainConfigs - error:", errorData);
  }
}
